...
function ensurePopupLayoutStyles() {
    ...
    style.textContent = `
        ...
        body.product-page-mode { background: var(--bg, #f8f5ef); overflow-x: hidden; }
        ...
        body.product-page-mode #product-modal #option { margin: 0 0 1.25rem; padding: 0; border: 0; }
        body.product-page-mode #product-modal #option .condition-chips { display: flex; flex-wrap: wrap; gap: .65rem; margin-top: .6rem; }
        body.product-page-mode #product-modal #option .condition-chip { min-width: 116px; min-height: 48px; padding: .65rem 1rem; border: 1px solid #8e867d; border-radius: 8px; background: #fff; color: #1c1b1a; font: inherit; font-weight: 700; cursor: pointer; }
        body.product-page-mode #product-modal #option .condition-chip.active { border: 2px solid #111; background: #f3f1ee; box-shadow: inset 0 0 0 1px #fff; }
        body.product-page-mode #product-modal #option .condition-chip:focus-visible { outline: 3px solid #3b82f6; outline-offset: 2px; }
        ...
        body.product-page-mode #product-modal .similar-product-card img { display: block; width: 100%; max-width: 100%; height: 150px; aspect-ratio: 1 / 1; object-fit: contain; background: #f4f1ec; }
        body.product-page-mode #product-modal .similar-product-title { min-height: 2.7em; padding: .65rem .7rem .2rem; overflow: hidden; font-size: .9rem; line-height: 1.3; font-weight: 700; overflow-wrap: anywhere; }
        body.product-page-mode #product-modal .similar-product-price { padding: .2rem .7rem .75rem; color: #1f7a46; font-size: .95rem; font-weight: 800; }
        body.product-page-mode #product-modal #similar-products-controls { display: flex; justify-content: center; gap: .55rem; margin-top: .8rem; }
...
(function initLightbox() {
    ...
    function openLightbox(urlsArray, startIndex) {
        ...
        /* ── Click a point in the image to zoom there; click again to reset ── */
        lbImg.style.cursor = "zoom-in";
        lbImg.style.transition = "transform .2s ease";
        lbImg.style.transformOrigin = "50% 50%";
        lbImg.style.transform = "scale(1)";
        lbImg.removeEventListener("click", lbImg._lbZoomClick);
        lbImg._lbZoomClick = (e) => {
            if (lbImg.style.transform === "scale(2)") {
                lbImg.style.transform = "scale(1)";
                lbImg.style.transformOrigin = "50% 50%";
                lbImg.style.cursor = "zoom-in";
                return;
        ...
        lbImg.addEventListener("click", lbImg._lbZoomClick);
    ...
    function closeLightbox() {
        ...
        if (lbImg._lbZoomClick) lbImg.removeEventListener("click", lbImg._lbZoomClick);
...
window.populateProductPopupById = async function populateProductPopupById(productId, opts) {
    ...
    let product = window.BondsmallCatalogAuthority && Array.isArray(window.BondsmallCatalogAuthority.records)
        ? window.BondsmallCatalogAuthority.records.find((item) => Number(item.id) === id) || null
        : null;
    if (!product && Array.isArray(window.products)) {
        product = window.products.find((item) => Number(item.id) === id) || null;
    }
