(() => {
  ...
  async function mount() { const response = await fetch("cart.html", { cache: "no-store" }); root = document.createElement("div"); root.innerHTML = await response.text(); const fragment = root.firstElementChild; document.body.appendChild(fragment); root = fragment; read(); render(); document.dispatchEvent(new Event("bonds-cart-ready")); }
  document.addEventListener("click", event => { const target = event.target.closest("[data-cart-action]"); if (target) { change(target.dataset.id, target.dataset.cartAction === "inc" ? 1 : -1); return; } if (event.target.closest("#sr-cart-btn,#cart-btn,#popup-cart-btn")) open(); if (event.target.closest("#close-cart")) close(); if (event.target.closest("#cart-backdrop")) close(); if (event.target.closest("#to-shipping")) document.dispatchEvent(new Event("bonds-checkout-start")); if (event.target.closest("#back-to-cart")) { document.dispatchEvent(new Event("bonds-checkout-back")); } });
