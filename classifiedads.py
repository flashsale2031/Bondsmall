...
def _profile_dir_for(profile_key):
    ...
    import hashlib
    base = os.path.join(os.path.dirname(os.path.abspath(__file__)), "chrome_profiles_ca")
...
def make_driver(headless: bool = False, profile_key=None) -> webdriver.Chrome:
    from selenium.webdriver.chrome.service import Service

    os.environ["SE_MANAGER_PATH"] = ""
    os.environ["WDM_SKIP_DOWNLOAD"] = "1"

    profile_dir = _profile_dir_for(profile_key) if profile_key else CHROME_PROFILE_DIR
...
def open_login_window(email, proxy_url=None):
    """Open a headed Chrome with the account's persistent profile at the ClassifiedAds login page.
    Uses webdriver_manager on Windows (matching craigslist_new.py approach) so it works locally."""
    import sys as _sys
    from selenium.webdriver.chrome.service import Service as _Service

    profile_dir = _profile_dir_for(email)
    ...
    # Find chromedriver: prefer webdriver_manager on Windows, else _find_binary
    ...
    if _sys.platform == "win32":
        try:
            ...
            print(f"  [ca-login] webdriver_manager failed: {e}")
    if not chromedriver_bin:
        chromedriver_bin = _find_binary(["chromedriver"], ["/usr/bin/chromedriver"])
    if not chromedriver_bin:
        raise RuntimeError("chromedriver not found. Install Chrome + webdriver-manager.")

    service = _Service(executable_path=chromedriver_bin)
...
def main():
    ...
    driver = make_driver(profile_key=email)
