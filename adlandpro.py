...
def _profile_dir_for(profile_key):
    ...
    import hashlib
    base = os.path.join(os.path.dirname(os.path.abspath(__file__)), "chrome_profiles_ap")
...
def open_login_window(email, proxy_url=None):
    """Open a headed Chrome with the account's persistent profile at the AdLandPro login page.
    ...
    if _sys.platform == "win32":
        try:
            ...
            print(f"  [ap-login] webdriver_manager failed: {e}")
    if not chromedriver_bin:
