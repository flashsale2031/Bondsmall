...
for path in sorted((ROOT/'catalog-pages').glob('products-page-*.js')):
    ...
    for p in json.loads('['+m.group(1)+']'):
        if str(p.get('category','')).strip().lower()=='jewelry':
...
page4=items[60:80]
out={'page':4,'page_size':20,'total_jewelry':len(items),'records':page4}
(ROOT/'jewelry-page4-image-audit.json').write_text(json.dumps(out,indent=2,ensure_ascii=False)+'\n')
